...
func TestStringLiteral(t *testing.T) {
	...
	if str.Value != "Hello World!" {
		t.Errorf("String has wrong value. got %q wanted %q",
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong number of parameters. got Parameters = %+v",
			fn.Parameters)
	}

	if fn.Parameters[0].String() != "x" {
		t.Fatalf("function has wrong parameter. got %q", fn.Parameters[0])
	}
...
func TestArrayIndexExpressions(t *testing.T) {
	tests := []struct {
		...
		{
			"let myArr = [1,2,3]; let i = myArr[0]; myArr[i];",
			2,
		},
		{
			"[1,2,3][3]",
...
func TestHashLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Eval didn't return Hash. got %T (%+v)",
			evaluated, evaluated)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong number of key/pair values. got %d, expected %d",
	...
	for expectedKey, expectedValue := range expected {
		...
		if !ok {
			t.Errorf("expected key not in Pairs")
		}
